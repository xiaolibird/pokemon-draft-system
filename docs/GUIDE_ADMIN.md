...
- **⚠️ 重要限制**: 
  - **禁止在倒计时进行中且剩余时间 > 5 秒时强制成交**
  - 系统会自动检测：如果倒计时仍在运行且剩余时间充足，**强制成交按钮将不会显示**
  - 这是为了防止管理员在选手仍在竞价时强行结束，确保公平性
  - 只有在手动模式（无倒计时）或倒计时剩余 ≤ 5 秒时，才会显示强制成交按钮
...
为防止误操作和系统滥用，系统对管理员和选手的操作进行了频率限制：

- **管理员操作**: 
  - 暂停/继续、撤销、跳过等控制操作有频率限制
  - 如果操作过于频繁，系统会返回限流提示，请稍后再试
  - 建议操作间隔 ≥ 1 秒
