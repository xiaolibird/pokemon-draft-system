...
export default function AdminSpectate() {
  ...
  const HISTORY_FETCH_FALLBACK_MS = 5000; // 兜底：即使 key 相同，也至少每 5 秒触发一次
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const keyChanged = lastHistoryKeyRef.current !== key;
    const throttleExpired =
      now - lastHistoryFetchAt.current >= HISTORY_FETCH_THROTTLE_MS;
    const fallbackExpired =
      now - lastHistoryFetchAt.current >= HISTORY_FETCH_FALLBACK_MS;

    // 满足任一条件即触发：key 变化、节流时间到、或兜底时间到
    const shouldFetch = keyChanged || throttleExpired || fallbackExpired;
  ...
  const handleControl = async (action: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const msg = (json as { error?: string })?.error || "操作失败";
        alert(msg);
                ...
                <>
                  ...
                  {contest.draftMode === "AUCTION" &&
                    ...
                    (() => {
                      ...
                      const remainingMs = contest.bidEndTime
                        ? new Date(contest.bidEndTime).getTime() - Date.now()
                        : 0;
                      const canForceFinalize =
                        isManual || !contest.bidEndTime || remainingMs <= 5000;
                      if (!canForceFinalize) return null;
                      return (
                        <>
                          {isManual && (
