  ...
  if (mode === "OPTIMIZED") {
    ...
    const contest = await prisma.contest.findUnique({
      ...
      select: {
        ...
        priceTiers: true, // SNAKE 模式必需，体积小总是查询
      },
    });
    ...
    // 3. Pokemon Pool: 只返回 ID、Status 和 basePrice (前端会 Smart Merge 保留静态数据)
    ...
    const pokemonPool = await prisma.pokemonPool.findMany({
      ...
      select: {
        id: true,
        status: true,
        basePrice: true, // PokemonCard 显示价格必需
      },
