  ...
  try {
    ...
    if (query) {
      ...
      if (psFilters.excludeTypes && psFilters.excludeTypes.length > 0) {
        andClauses.push({ NOT: { types: { hasSome: psFilters.excludeTypes } } })
      }
    ...
    if (ruleSet) {
      if (ruleSet === 'Regulation F' || ruleSet === 'Reg F') {
        andClauses.push({ tags: { has: 'reg-f' } })
      } else if (ruleSet === 'Regulation G' || ruleSet === 'Reg G') {
        andClauses.push({ tags: { has: 'reg-g' } })
      } else if (ruleSet === 'Regulation H' || ruleSet === 'Reg H') {
        andClauses.push({ tags: { has: 'reg-h' } })
      } else if (ruleSet === 'None') {
        // No tag filtering
      }
    }

    // Resilient Exclusion Logic removed as per user request
